$(function(){
	...
	$('.next').on("click", function(){
		...
		if (i == $('.rslides img').length) {
			i=0;
		}
		//set current image and previous image
		var currentImg = $('.rslides img').eq(i);
		var prevImg = $('.rslides img').eq(i-1);
	...
	$('.previous').on("click", function(){
		...
		if (i==0) {	
			prevImg = $('.rslides img').eq(0);
			i=$('.rslides img').length-1;
			currentImg = $('.rslides img').eq(i);
		}
